...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=['hifigan', 'hn-sinc-nsf', 'hn-sinc-nsf-hifi', 'waveglow', 'melgan'], augmentation_methods=[], num_additional_real=2, trim_length=64000, wav_samp_rate=16000):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        additional_idxs = np.random.choice(idxs, len(self.vocoders) - 1 - len(self.augmentation_methods), replace=False)
        ...
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + augmented_audios + additional_audios + vocoded_audios
