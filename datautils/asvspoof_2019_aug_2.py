...
class Dataset_for(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for vf in vocoded_audio_files:
            ...
            # augment the vocoded samples, random choose one augmentation method
            augmethod_index = random.choice(range(len(self.augmentation_methods)))
            augmented_vocoded_audio = globals()[self.augmentation_methods[augmethod_index]](vocoded_audio, self.args, self.sample_rate, audio_path = vf)
        ...
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + augmented_audios + additional_audios + augmented_additional_audios + vocoded_audios + augmented_vocoded_audios
        ...
        label = [1] * (len(augmented_audios) +len(additional_audios) + len(augmented_additional_audios) + 1) + [0] * (len(vocoded_audios) + len(augmented_vocoded_audios))
        # print("label", label)
